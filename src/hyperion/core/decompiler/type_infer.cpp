...
struct TypeInfer::SigMap {
    // Signatures are copied by value: the analysis DB (and its unordered_map
    // nodes) can be freed between decompiles while this cache lives on.
    std::unordered_map<std::string, FuncSignature> by_name;
...
void TypeInfer::seed_from_db(const AnalysisDB& db, va_t entry) {
    db_funcs_.clear();
    db_entry_ = entry;

    // Rebuild when the image identity changes (new load / reanalysis). Heap
    // address reuse is defeated by including content sizes in the key, and
    // correctness never depends on the key because entries are owned copies.
    static thread_local std::shared_ptr<SigMap> cache;
    const bool stale = !cache || cache->db != &db ||
                       cache->signature_count != db.signatures.size() ||
                       cache->image_base != db.image_base ||
                       cache->function_count != db.funcs.size() ||
                       cache->instruction_count != db.insns.size();
    if (stale) {
        auto fresh = std::make_shared<SigMap>();
        fresh->db = &db;
        fresh->signature_count = db.signatures.size();
        fresh->image_base = db.image_base;
        fresh->function_count = db.funcs.size();
        fresh->instruction_count = db.insns.size();
        for (const auto& [func_entry, signature] : db.signatures) {
            const std::string* name = nullptr;
            auto name_it = db.names.find(func_entry);
            if (name_it != db.names.end() && !name_it->second.empty()) {
                name = &name_it->second;
            } else {
                auto func_it = db.funcs.find(func_entry);
                if (func_it != db.funcs.end() && !func_it->second.name.empty())
                    name = &func_it->second.name;
            }
            if (name) fresh->by_name[*name] = signature;
        }
...
const KnownFunc* TypeInfer::find_known(const std::string& name) const {
    // Analyzer-recovered signatures win over the hardcoded table.
    if (db_seeded_ && db_sig_map_) {
        auto it = db_sig_map_->by_name.find(name);
        if (it != db_sig_map_->by_name.end()) {
            const FuncSignature& sig = it->second;
            KnownFunc kf;
            kf.name = name;
            if (sig.return_type == "void")
                kf.ret_type = DecompType::make_void();
            else
                kf.ret_type = DecompType::make_int(64);
            for (size_t i = 0; i < sig.param_types.size(); ++i) {
                kf.param_types.push_back(DecompType::make_int(64));
                kf.param_names.push_back(i < sig.param_names.size()
                                             ? sig.param_names[i]
                                             : fmt::format("a{}", i));
            }
            db_funcs_.push_back(std::move(kf));
            return &db_funcs_.back();
        }
    }
...
void TypeInfer::run(PcodeFunc& func) {
    ...
    // Function return type: analyzer-recovered signature first.
    if (db_seeded_ && db_sig_map_) {
        auto it = db_sig_map_->by_name.find(func.name);
        if (it != db_sig_map_->by_name.end()) {
            ret_type_ = it->second.return_type == "void"
                            ? DecompType::make_void()
                            : DecompType::make_int(64);
        }
    }
}
