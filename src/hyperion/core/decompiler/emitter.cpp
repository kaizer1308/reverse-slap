...
namespace {
int expression_precedence(PcodeOp op) {
    switch (op) {
    case PcodeOp::BOOL_OR: return 1;
    case PcodeOp::BOOL_AND: return 2;
    case PcodeOp::OR: return 3;
    case PcodeOp::XOR: return 4;
    case PcodeOp::AND: return 5;
    case PcodeOp::INT_EQUAL: case PcodeOp::INT_NEQUAL: return 6;
    case PcodeOp::INT_LESS: case PcodeOp::INT_SLESS: return 7;
    case PcodeOp::SHIFT_LEFT: case PcodeOp::SHIFT_RIGHT: return 8;
    case PcodeOp::ADD: case PcodeOp::SUB: return 9;
    case PcodeOp::INT_MULT: case PcodeOp::INT_DIV: return 10;
    default: return 11;
    }
}

bool is_binary_expression(const CExpr& expr) {
    return expr.args.size() == 2 && expression_precedence(expr.op) < 11;
}
}
...
std::string Emitter::expr_str(const CExpr& e) {
    ...
    if (e.op == PcodeOp::CALL || e.op == PcodeOp::INTRINSIC) {
    ...
    if ((e.op == PcodeOp::BOOL_NOT || e.op == PcodeOp::INT_NEGATE || e.op == PcodeOp::INT_NOT) &&
        !e.args.empty()) {
        std::string inner = expr_str(e.args[0]);
        if (is_binary_expression(e.args[0])) inner = fmt::format("({})", inner);
        return fmt::format("{}{}", e.op == PcodeOp::BOOL_NOT ? "!" : "~", inner);
    }
    ...
    if (e.args.size() == 2) {
        ...
        case PcodeOp::SHIFT_ARIGHT: op = ">>"; break;
        case PcodeOp::ROTATE_LEFT: op = "rol"; break;
        case PcodeOp::ROTATE_RIGHT: op = "ror"; break;
        case PcodeOp::INT_MULT: op = "*"; break;
        case PcodeOp::INT_DIV: case PcodeOp::INT_SDIV: op = "/"; break;
        ...
        const int precedence = expression_precedence(e.op);
        if (is_binary_expression(e.args[0]) && expression_precedence(e.args[0].op) < precedence)
            l = fmt::format("({})", l);
        if (is_binary_expression(e.args[1]) && expression_precedence(e.args[1].op) <= precedence)
            r = fmt::format("({})", r);
...
std::string Emitter::cond_str(const CExpr& e) {
    if (e.is_var()) {
        // Single-use temps inline into the condition too; otherwise the
        // defining statement is suppressed but the use prints a bare tN.
        if (e.vn.is_temp() && inline_temps_.count(e.vn.id)) {
            auto dit = temp_defs_.find(e.vn.id);
            if (dit != temp_defs_.end())
                return expr_str(dit->second->expr);
        }
        if (e.vn.is_temp() && inline_temps_.count(e.vn.id)) {
            auto dit = temp_defs_.find(e.vn.id);
    ...
    if (e.op == PcodeOp::INT_SLESS && e.args.size() == 2)
        return fmt::format("{} < {}", expr_str(e.args[0]), expr_str(e.args[1]));
    if (e.op == PcodeOp::BOOL_OR && e.args.size() == 2)
        return fmt::format("{} || {}", expr_str(e.args[0]), expr_str(e.args[1]));
    if (e.op == PcodeOp::BOOL_AND && e.args.size() == 2)
        return fmt::format("{} && {}", expr_str(e.args[0]), expr_str(e.args[1]));
    return expr_str(e);
}
...
bool Emitter::is_dead_stmt(const CStmt& s) const {
    ...
    if (s.kind == StmtKind::Label || s.kind == StmtKind::Goto) return false;
    if (s.kind == StmtKind::Expr) {
        if (s.expr.op == PcodeOp::CALL || s.expr.op == PcodeOp::INTRINSIC || s.expr.op == PcodeOp::STORE) return false;
        return true;
    }
    if (s.kind == StmtKind::Assign) {
        if (s.expr.op == PcodeOp::CALL || s.expr.op == PcodeOp::INTRINSIC) return false;
...
void Emitter::build_inline_map(const CFunc& func) {
    ...
    std::function<void(const std::vector<CStmt>&)> scan_defs = [&](const std::vector<CStmt>& stmts) {
        for (auto& s : stmts) {
            if (s.kind == StmtKind::Assign && s.dst.is_temp() &&
                s.expr.op != PcodeOp::CALL && s.expr.op != PcodeOp::INTRINSIC &&
                // Loads must not inline: moving them to the use site can cross
                // stores/calls and change which memory value is read.
                s.expr.op != PcodeOp::LOAD)
...
bool Emitter::is_trailing_return(const CStmt& s) const {
    return s.kind == StmtKind::Return && is_void_func_;
}
...
void Emitter::emit_stmt(const CStmt& s, int indent, std::vector<PseudoLine>& out, bool& returned) {
    ...
    case StmtKind::Assign: {
        ...
        if (s.expr.op == PcodeOp::INTRINSIC) {
            out.push_back({indent, s.expr.call_name + ";", s.addr});
            break;
        }
    ...
    case StmtKind::Return: {
        ...
        const std::string rv = expr_str(s.expr);
        if (rv.empty())
            out.push_back({indent, "return;", s.addr});
        else
    ...
    case StmtKind::Label:
        out.push_back({std::max(0, indent - 1), fmt::format("loc_{:X}:", s.goto_target), s.addr});
        break;
    case StmtKind::Goto:
