    ...
    for (int bi = 0; bi < (int)func.blocks.size(); ++bi) {
        ...
        for (int oi = 0; oi < (int)blk.ops.size(); ++oi) {
            ...
            for (auto& in : blk.ops[oi].inputs) {
                if (in.kind == old_vn.kind && in.id == old_vn.id &&
                    in.offset == old_vn.offset && in.size == old_vn.size)
                    in = new_vn;
            }
        }
    }
...
void Propagate::constant_fold(PcodeFunc& func) {
    for (auto& blk : func.blocks) {
        for (auto& op : blk.ops) {
            ...
            case PcodeOp::SHIFT_ARIGHT: result = r < 64 ? static_cast<u64>(static_cast<i64>(l) >> r) : (static_cast<i64>(l) < 0 ? ~u64(0) : 0); break;
            case PcodeOp::ROTATE_LEFT: result = std::rotl(l, static_cast<int>(r & 63)); break;
            case PcodeOp::ROTATE_RIGHT: result = std::rotr(l, static_cast<int>(r & 63)); break;
    ...
    for (int pass = 0; pass < 12; ++pass) {
        ...
        for (int bi = 0; bi < (int)func.blocks.size(); ++bi) {
            ...
            for (int oi = 0; oi < (int)blk.ops.size(); ++oi) {
                ...
                if (op.op == PcodeOp::CALL || op.op == PcodeOp::INTRINSIC || op.op == PcodeOp::RETURN ||
...
void Propagate::eliminate_identity(PcodeFunc& func) {
    for (auto& blk : func.blocks) {
        for (auto& op : blk.ops) {
            if (op.op != PcodeOp::COPY || op.seq == -1) continue;
            if (op.inputs.empty()) continue;
            const auto& src = op.inputs[0];
            if (op.output == src)
                op.op = PcodeOp::NOP;
        }
        std::erase_if(blk.ops, [](const PcodeInsn& op) { return op.op == PcodeOp::NOP; });
