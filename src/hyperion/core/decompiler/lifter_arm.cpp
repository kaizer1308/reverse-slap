...
void LifterARM::lift_insn(const Insn& insn, PcodeBlock& blk, const AnalysisDB& db) {
    ...
    if (mn == "movt") {
        // movt inserts into the upper 16 bits; keep the low half.
        if (insn.op_count >= 2) {
            ...
            emit(blk, PcodeOp::AND, cleared, {old, vn_const(0xFFFF, old.size)});
            Varnode placed = alloc_temp(old.size);
            emit(blk, PcodeOp::OR, placed, {cleared, vn_const(imm << 16, old.size)});
            operand_write(insn, 0, placed, blk);
        }
    } else if (mn == "mov" || mn == "movw") {
