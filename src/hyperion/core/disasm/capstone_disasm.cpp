...
bool CapstoneDisasm::decode(va_t addr, const u8* data, size_t len, Insn& out) {
    ...
    if (insn->detail) {
        ...
        if (impl_->arch == Arch::ARM64) {
            auto& arm64 = d->arm64;
            out.op_count = (std::min)(static_cast<u8>(arm64.op_count), static_cast<u8>(4));
            for (u8 i = 0; i < out.op_count; ++i) {
                auto& cop = arm64.operands[i];
                auto& oop = out.ops[i];
                oop.read = (cop.access & CS_AC_READ) != 0;
                oop.write = (cop.access & CS_AC_WRITE) != 0;
                switch (cop.type) {
                case ARM64_OP_REG:
                    oop.type = OpType::Reg;
            ...
            auto& arm = d->arm;
            out.op_count = (std::min)(static_cast<u8>(arm.op_count), static_cast<u8>(4));
            for (u8 i = 0; i < out.op_count; ++i) {
