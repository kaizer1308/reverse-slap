...
namespace ioctl_codes {
    ...
    // Clean-shutdown pair (driver-side sd_ident_request / sd_shutdown_request)
    __forceinline DWORD IDENT()   { return make(63); }
    __forceinline DWORD SHUTDOWN(){ return make(64); }
    __forceinline DWORD LOGCTL()  { return make(65); }
}
...
namespace voyager {
    namespace detail {
        ...
        __forceinline void append_ioctl_raw_debug_log(const char* msg) noexcept {
            ...
            *(slash + 1) = '\0';
            char path[MAX_PATH] = {};
            _snprintf_s(path, sizeof(path), _TRUNCATE, "%sslop_debug.log", module);
            // hard cap, past 8 mb the file stops growing until the next session
            WIN32_FILE_ATTRIBUTE_DATA fad = {};
            if (GetFileAttributesExA(path, GetFileExInfoStandard, &fad) &&
                (static_cast<unsigned long long>(fad.nFileSizeHigh) << 32 |
                 fad.nFileSizeLow) > 8ull * 1024 * 1024)
                return;
        ...
        // failure telemetry only, writing every successful ioctl made this
// file grow without bound under scans, ok path stays on
// outputdebugstring
        __forceinline void debug_ioctl_raw_log(const char* phase,
                                               const raw_ioctl_telemetry& t) noexcept {
            char buf[2048];
            _snprintf_s(buf, sizeof(buf), _TRUNCATE,
                "[SLOP-IOCTL-RAW] %s requested=0x%08X size=%u bytes=%u gle=%u connected=%u handle=0x%llX attached_pid=%u req_pid=%u req_tid=%u local_pid=%u local_tid=%u elapsed_ms=%llu",
                phase ? phase : "?",
                t.requested_code,
                t.buffer_size,
                t.bytes_returned,
            ...
            OutputDebugStringA("\n");
            if (t.gle != 0 || std::strncmp(phase ? phase : "", "OK", 2) != 0) {
        ...
        // Clean-shutdown pair (ABI-synced with driver Struct.h)
        struct ident_request {
            std::uint32_t driver_version;
            std::uint32_t unloading;
            std::uint32_t service_path_len;
            std::uint32_t reserved;
            wchar_t      service_path[260];
        };
        static_assert(sizeof(ident_request) == 536, "ident_request size mismatch with kernel driver");

        struct shutdown_request {
            std::uint32_t magic;
            std::uint32_t status;
        };
        static_assert(sizeof(shutdown_request) == 8, "shutdown_request size mismatch with kernel driver");

        // LOGCTL: live log level + cap control (query or apply)
        struct logctl_request {
            std::uint32_t magic;      // 0x5D10C0DE to apply, 0 to query
            std::uint32_t log_level;  // in/out: 0..4
            std::uint32_t log_cap_mb; // in/out: 1..512
            std::uint32_t status;
        };
        static_assert(sizeof(logctl_request) == 16, "logctl_request size mismatch with kernel driver");
...
#pragma pack(push, 8)
        ...
        static constexpr std::uint32_t WFP_SLOP_MATCH_SUBLAYER = 0x00000001u;
        static constexpr std::uint32_t WFP_SLOP_MATCH_ACTION_CALLOUT = 0x00000002u;
        static constexpr std::uint32_t WFP_SLOP_MATCH_DISPLAY_DATA = 0x00000004u;
        static constexpr std::uint32_t WFP_SLOP_MATCH_RUNTIME_FALLBACK = 0x80000000u;
        ...
        struct wfp_callout_entry {
            ...
            std::uint32_t slop_match_reason;
...
#pragma pack(pop)
    ...
    public:
        ...
        struct driver_identity {
            ...
            std::wstring service_path;   // registry key of the loaded service
        };

        // ask the driver for its identity, chiefly the service registry key
        // created)
        bool query_driver_identity(driver_identity& out) noexcept;

        // Arm the driver's quiesce flag: from here on every IOCTL except
        // IDENT is refused with STATUS_DELETE_PENDING. Call before closing
        // the device handle and unloading the service
        bool arm_shutdown() noexcept;

        struct log_config {
            std::uint32_t level = 0;      // 0..4 (0=errors, 1=critical+life,
                                          // 2=+per-IOCTL, 3=+per-packet, 4=all)
            std::uint32_t cap_mb = 0;     // log size cap in MB
        };
        // Query the driver's log configuration. `apply` set to true with
        // values in `cfg` applies them first (out reflects the result)
        bool log_config_op(log_config& cfg, bool apply) noexcept;
