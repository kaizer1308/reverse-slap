#include "voyager_comm.h"
#include "core/infra/diag.hpp"

namespace slop { namespace core { namespace infra { namespace diag {

inline void log_tagged_fmt(const char* tag, const char* fmt, ...) noexcept {
    char buf[2048];
    va_list ap;
    va_start(ap, fmt);
    _vsnprintf_s(buf, sizeof(buf), _TRUNCATE, fmt, ap);
    va_end(ap);
    log(level_t::trace, tag, buf);
}

inline void log_tagged_critical_fmt(const char* tag, const char* fmt, ...) noexcept {
    char buf[2048];
    va_list ap;
    va_start(ap, fmt);
    _vsnprintf_s(buf, sizeof(buf), _TRUNCATE, fmt, ap);
    va_end(ap);
    log(level_t::error, tag, buf);
}

}}}}

namespace diag = slop::core::infra::diag;
#include "voyager_comm.h"
#include <algorithm>
#include <cctype>
#include <chrono>
...
#ifdef _DEBUG
#define RC_UM_DBG(fmt, ...) do { char _rc_buf[512]; _snprintf_s(_rc_buf, sizeof(_rc_buf), _TRUNCATE, "[SLOP-RC-UM] " fmt "\n", ##__VA_ARGS__); OutputDebugStringA(_rc_buf); } while(0)
#else
#define RC_UM_DBG(fmt, ...) do { char _rc_buf[512]; _snprintf_s(_rc_buf, sizeof(_rc_buf), _TRUNCATE, "[SLOP-RC-UM] " fmt "\n", ##__VA_ARGS__); OutputDebugStringA(_rc_buf); } while(0)
#endif
...
namespace {
    // 64 kb chunks cut the ioctl round trips sixteen fold without changing semantics
    constexpr std::size_t k_staged_physical_chunk_size = 0x10000;

    // thread local staging buffer, the driver dereferences it inside the ioctl
    thread_local std::uint8_t t_phys_staging[k_staged_physical_chunk_size];
    static_assert(k_staged_physical_chunk_size <= 0x4000000,
                  "client chunk must stay within the driver's PHYS size cap");

    std::uint8_t* phys_staging() noexcept { return t_phys_staging; }

    // context writes must run against a suspended thread or a stale snapshot kills the target
    class thread_freeze_guard_t final {
    public:
        thread_freeze_guard_t(voyager::device_t& dev, std::uint32_t tid) noexcept
            : dev_(dev), tid_(tid) {
            ok_ = dev.suspend_thread(tid);
        }
        ~thread_freeze_guard_t() {
            if (ok_) dev_.resume_thread(tid_);
        }
        thread_freeze_guard_t(const thread_freeze_guard_t&) = delete;
        thread_freeze_guard_t& operator=(const thread_freeze_guard_t&) = delete;
...
bool voyager::device_t::query_driver_identity(driver_identity& out) noexcept {
    out = {};
    if (!is_connected()) return false;

    detail::ident_request req{};
    if (!send_request(ioctl_codes::IDENT(), &req, sizeof(req)))
        return false;
...
bool voyager::device_t::arm_shutdown() noexcept {
    ...
    detail::shutdown_request req{};
    req.magic = 0x5D100D0C;   // 'SD' + shutdown marker (driver-side constant)
    if (!send_request(ioctl_codes::SHUTDOWN(), &req, sizeof(req)))
        return false;
    return req.status == 0;
}

bool voyager::device_t::log_config_op(log_config& cfg, bool apply) noexcept {
    if (!is_connected()) return false;

    detail::logctl_request req{};
    if (apply) {
        req.magic = 0x5D10C0DE;
        req.log_level = cfg.level;
        req.log_cap_mb = cfg.cap_mb;
    }
    if (!send_request(ioctl_codes::LOGCTL(), &req, sizeof(req)))
        return false;
    if (req.status != 0) return false;

    cfg.level = req.log_level;
    cfg.cap_mb = req.log_cap_mb;
    return true;
}
...
std::size_t voyager::device_t::transfer_physical_read(
    ...
    std::uint8_t* const staging = phys_staging();
    ...
    while (total_read < size) {
        ...
        std::memset(staging, 0, chunk_size);
        ...
        std::memcpy(destination + total_read, staging, bytes_read);
...
std::size_t voyager::device_t::transfer_physical_write(
    ...
    while (total_written < size) {
        ...
        std::memcpy(staging, source + total_written, chunk_size);
...
bool voyager::device_t::set_hardware_breakpoint(std::uint32_t tid, int index, std::uint64_t address, int type, int size) noexcept {
    ...
    // freeze for the whole read modify write, a stale rip restore kills the thread
    thread_freeze_guard_t freeze(*this, tid);
    if (!freeze.ok()) {
        diag::log_tagged_fmt("comm",
            "HWBP set suspend_failed pid=%u tid=%u index=%d address=0x%llX",
            process_id_, tid, index,
            static_cast<unsigned long long>(address));
        SetLastError(ERROR_ACCESS_DENIED);
        return false;
    }
...
bool voyager::device_t::clear_hardware_breakpoint(std::uint32_t tid, int index) noexcept {
    ...
    // same freeze discipline, the clear writes a full context too
    thread_freeze_guard_t freeze(*this, tid);
    if (!freeze.ok()) {
        diag::log_tagged_fmt("comm",
            "HWBP clear suspend_failed pid=%u tid=%u index=%d",
            process_id_, tid, index);
        SetLastError(ERROR_ACCESS_DENIED);
        return false;
    }
...
voyager::device_t::enumerate_wfp_callouts(const std::string& filter_module) noexcept {
    ...
    if (send_request(ioctl_codes::EWFP(), req, static_cast<DWORD>(sizeof(*req)))) {
        ...
        for (std::uint32_t i = 0; i < req->callout_count; i++) {
            ...
            info.slop_match_reason = e.slop_match_reason;
