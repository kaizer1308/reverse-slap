...
namespace mem_guard {
    ...
    __forceinline void log_page_walk(const char* phase, UINT32 pid, UINT64 dtb, UINT64 va, const page_walk_diag_t& d, UINT64 strong_pa) {
        SD_LOG("PHYS_RW_PAGE_WALK phase=%s pid=%lu dtb=0x%llx va=0x%llx strong_pa=0x%llx walk_pa=0x%llx level=%lu remaining=0x%llx pml4_ok=%u pdpt_ok=%u pde_ok=%u pte_ok=%u present=%u pml4e=0x%llx pdpte=0x%llx pde=0x%llx pte=0x%llx pml4_p=%u pml4_w=%u pml4_u=%u pml4_nx=%u pdpt_p=%u pdpt_w=%u pdpt_u=%u pdpt_l=%u pdpt_nx=%u pde_p=%u pde_w=%u pde_u=%u pde_l=%u pde_nx=%u pte_p=%u pte_w=%u pte_u=%u pte_nx=%u",
            phase ? phase : "<null>",
            (ULONG)pid,
            dtb,
            va,
...
NTSTATUS functions::handle777e(p_physical_rw request) {
    ...
    if (!request) {
        SD_LOG("PHYS_RW_REJECT reason=request_null status=0x%08X cpu=%lu irql=%lu qpc=%lld tsc=%llu",
    ...
    SD_LOG("PHYS_RW_ENTER pid=%lu dtb=0x%llx va=0x%llx size=0x%llx should_write=%u buffer=0x%llx requestor_pid=%llu current_pid=%llu current_tid=%llu irql=%lu qpc=%lld tsc=%llu",
        (ULONG)request->pid,
        request->dtb,
    ...
    if (!request->buffer || request->size == 0) {
        SD_LOG("PHYS_RW_EXIT pid=%lu dtb=0x%llx va=0x%llx size=0x%llx should_write=%u status=0x%08X ret=0 reason=bad_buffer_or_size elapsed_us=%llu",
            (ULONG)request->pid,
            request->dtb,
            (UINT64)request->address,
            (UINT64)request->size,
    ...
    if (request->dtb == 0) {
        SD_LOG("PHYS_RW_EXIT pid=%lu dtb=0x%llx va=0x%llx size=0x%llx should_write=%u status=0x%08X ret=0 reason=dtb_zero elapsed_us=%llu",
            (ULONG)request->pid,
            request->dtb,
            (UINT64)request->address,
            (UINT64)request->size,
    ...
    if (request->size > 0x4000000) {
        SD_LOG("PHYS_RW_EXIT pid=%lu dtb=0x%llx va=0x%llx size=0x%llx should_write=%u status=0x%08X ret=0 reason=size_too_large elapsed_us=%llu",
            (ULONG)request->pid,
            request->dtb,
            (UINT64)request->address,
            (UINT64)request->size,
    ...
    if (target_addr == 0) {
        SD_LOG("PHYS_RW_EXIT pid=%lu dtb=0x%llx va=0x%llx size=0x%llx should_write=%u status=0x%08X ret=0 reason=target_zero elapsed_us=%llu",
            (ULONG)request->pid,
            request->dtb,
            (UINT64)request->address,
            (UINT64)request->size,
    ...
    while (remaining_size > 0) {
        ...
        if (physical_address) {
            ...
            if (is_write) {
                // COW-safe attach write. Raw physical writes are FORBIDDEN here:
                // they bypass copy-on-write, so writing a shared image page
                // (kernel32, ntdll, ...) stomps the prototype page for EVERY
                // process in the system. Going through the attached address
                // space lets the MMU apply COW and read-only protection exactly
                // as user-mode would — read-only pages get refused, COW pages
                // get private copies. SW breakpoints in image code must use
                // hardware breakpoints (DR regs) instead.
                    ...
                    mem_guard::is_valid_user_range(current_virtual_address + transfer_size - 1))
                {
                    ...
                    if (target_proc) {
                        ...
                        if (!km_verify) {
                            km_verify = ExAllocatePool2(POOL_FLAG_NON_PAGED, 0x1000, 'vFwW');
                        }

                        if (km_staging && km_verify) {
                            BOOLEAN stage_ok = FALSE;
                            __try {
                                strong::kmemcpy(km_staging,
                            ...
                            if (stage_ok) {
                                KAPC_STATE write_apc{};
                                BOOLEAN write_ok = FALSE;
                                BOOLEAN verify_ok = FALSE;

                                if (_KeStackAttachProcess) { _KeStackAttachProcess(target_proc, &write_apc); }
                                __try {
                                    ProbeForWrite((PVOID)current_virtual_address, transfer_size, 1);
                                    strong::kmemcpy((PVOID)current_virtual_address, km_staging, transfer_size);
                                    write_ok = TRUE;
                                    strong::kmemcpy(km_verify, (PVOID)current_virtual_address, transfer_size);
                                    verify_ok = RtlCompareMemory(km_verify, km_staging, transfer_size) == transfer_size;
                                }
                                __except (EXCEPTION_EXECUTE_HANDLER) {
                                    write_ok = FALSE;
                                }
                                if (_KeUnstackDetachProcess) { _KeUnstackDetachProcess(&write_apc); }

                                SD_LOG("PHYS_RW_ATTACH_WRITE pid=%lu va=0x%llx requested=0x%llx written=%u verified=%u elapsed_us=%llu",
                                    (ULONG)target_pid,
                ...
                operation_status = STATUS_UNSUCCESSFUL;
                bytes_transferred = 0;
            }
            else {
            ...
            SD_LOG("PHYS_RW_CHUNK pid=%lu dtb=0x%llx va=0x%llx pa=0x%llx requested=0x%llx should_write=%u status=0x%08X bytes=0x%llx total_before=0x%llx remaining_before=0x%llx elapsed_us=%llu",
                (ULONG)target_pid,
                process_dir_base,
                current_virtual_address,
                physical_address,
            ...
            SD_LOG("PHYS_RW_TRANSLATE_FAIL pid=%lu dtb=0x%llx va=0x%llx size=0x%llx should_write=%u total=0x%llx remaining=0x%llx walk_present=%u walk_level=%lu walk_pa=0x%llx elapsed_us=%llu",
                (ULONG)target_pid,
                process_dir_base,
                current_virtual_address,
                (UINT64)transfer_size,
            ...
            mem_guard::is_valid_user_range(current_virtual_address + transfer_size - 1))
        {
            if (!proc_lookup_attempted) {
                ...
                NTSTATUS lookup_status = _PsLookupProcessByProcessId
                    ? _PsLookupProcessByProcessId((HANDLE)(ULONG_PTR)target_pid, &target_proc)
                    : STATUS_NOT_SUPPORTED;
                SD_LOG("PHYS_RW_SOFTFAULT_LOOKUP pid=%lu status=0x%08X process=%p va=0x%llx elapsed_us=%llu",
                    (ULONG)target_pid,
                    (ULONG)lookup_status,
                    target_proc,
                    current_virtual_address,
                ...
                if (km_staging) {
                    ...
                    if (read_ok && bytes_staged > 0) {
                        __try {
                            ...
                            chunk_done = TRUE;
                            final_status = STATUS_SUCCESS;
                            SD_LOG("PHYS_RW_SOFTFAULT_CHUNK pid=%lu va=0x%llx requested=0x%llx bytes=0x%llx total=0x%llx elapsed_us=%llu",
                                (ULONG)target_pid,
                                current_virtual_address,
                                (UINT64)transfer_size,
                                (UINT64)bytes_staged,
        ...
        if (is_write) {
            ...
            SD_LOG("PHYS_RW_WRITE_FAIL_CLOSED pid=%lu dtb=0x%llx va=0x%llx requested=0x%llx total=0x%llx remaining=0x%llx elapsed_us=%llu",
                (ULONG)target_pid,
                process_dir_base,
                current_virtual_address,
                (UINT64)transfer_size,
        ...
        SD_LOG("PHYS_RW_READ_ZERO_FILL pid=%lu dtb=0x%llx va=0x%llx requested=0x%llx total_before=0x%llx remaining_before=0x%llx elapsed_us=%llu",
            (ULONG)target_pid,
            process_dir_base,
            current_virtual_address,
            (UINT64)transfer_size,
    ...
    if (km_verify) {
        ExFreePoolWithTag(km_verify, 'vFwW');
    }
    if (target_proc) {
    ...
    SD_LOG("PHYS_RW_EXIT pid=%lu dtb=0x%llx va=0x%llx size=0x%llx should_write=%u status=0x%08X ret=0x%llx remaining=0x%llx elapsed_us=%llu",
        (ULONG)target_pid,
        process_dir_base,
        target_addr,
        (UINT64)request->size,
