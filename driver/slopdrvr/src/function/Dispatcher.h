...
namespace ioctl_codes {
    ...
    // Clean-shutdown pair: IDENT hands the client the service registry path
    // (so it can NtUnloadDriver the exact randomized key the mapper made),
    // SHUTDOWN arms the quiesce flag so the client can detach before unload.
    __forceinline ULONG IDENT()   { return make(63); }
    __forceinline ULONG SHUTDOWN(){ return make(64); }

    // Runtime log control (level + cap), since the registry values live in
    // the mapper's ephemeral service key and can't be set before load.
    __forceinline ULONG LOGCTL()  { return make(65); }
}

namespace sd_state {
    // Unload lifecycle. g_unloading flips in DriverUnload (and early via the
    // SHUTDOWN ioctl) — the dispatcher rejects new work from that point on.
    inline volatile LONG g_unloading = 0;
    // In-flight IOCTL count; DriverUnload waits (bounded) for it to drain.
    inline volatile LONG g_active_ioctls = 0;
}
    ...
    __forceinline NTSTATUS Pilot(PDEVICE_OBJECT device_object, PIRP irp) {
        ...
        if (malware_safe::any_sandboxed()) {
            ...
            if (malware_safe::is_sandboxed_pid(caller_pid, &flags) &&
                ...
                SD_MALSAFE_LOG_INFO("DENY kernel_device_open pid=%lu flags=0x%08X status=0x%08X",
    ...
    __forceinline NTSTATUS Controller(PDEVICE_OBJECT device_object, PIRP irp) {
        ...
        if (!buffer) {
            if (code == ioctl_codes::STRM() || code == ioctl_codes::CKIL()) {
                SD_LOG("netaction::DISPATCH null_buffer code=0x%08X input_size=%lu output_size=%lu",
        ...
        if (malware_safe::any_sandboxed()) {
            ...
            if (!caller_validation::is_registered_client(caller_pid)) {
                ...
                if (malware_safe::is_sandboxed_pid(caller_pid, &sbx_flags) &&
                    ...
                    malware_safe::record_denial(caller_pid);
                    SD_MALSAFE_LOG_INFO("DENY ioctl_from_sandbox pid=%lu flags=0x%08X status=0x%08X",
        ...
        // IDENT is the one code honored while quiescing (the client polls it
        // to observe the state); everything else is refused so no new work
        // can start against a driver that is going away.
        if (sd_state::g_unloading && code != ioctl_codes::IDENT()) {
            SD_LOG("DISPATCH reject_unloading code=0x%08X", code);
            irp->IoStatus.Status = STATUS_DELETE_PENDING;
            irp->IoStatus.Information = 0;
            _IofCompleteRequest(irp, IO_NO_INCREMENT);
            return STATUS_DELETE_PENDING;
        }
        _InterlockedIncrement(&sd_state::g_active_ioctls);
        struct ioctl_guard_t {
            ~ioctl_guard_t() { _InterlockedDecrement(&sd_state::g_active_ioctls); }
        } ioctl_guard;

        if (code == ioctl_codes::IDENT()) {
            if (input_size >= sizeof(_SD_IDENT) && output_size >= sizeof(_SD_IDENT)) {
                p_sd_ident_request ident = (p_sd_ident_request)buffer;
                RtlZeroMemory(ident, sizeof(*ident));
                ident->driver_version = 1;
                ident->unloading = _InterlockedCompareExchange(&sd_state::g_unloading, 0, 0);
                if (service_identity::is_captured()) {
                    const wchar_t* path = service_identity::get();
                    ULONG n = 0;
                    while (path[n] && n < 259) { ident->service_path[n] = path[n]; ++n; }
                    ident->service_path[n] = L'\0';
                    ident->service_path_len = (n + 1) * sizeof(WCHAR);
                }
                status = STATUS_SUCCESS;
                bytes = sizeof(_SD_IDENT);
            }
            else {
                status = STATUS_INFO_LENGTH_MISMATCH;
            }
        }
        else if (code == ioctl_codes::SHUTDOWN()) {
            if (input_size >= sizeof(_SD_SHUTDOWN) && output_size >= sizeof(_SD_SHUTDOWN)) {
                p_sd_shutdown_request req = (p_sd_shutdown_request)buffer;
                if (req->magic == 0x5D100D0C) {
                    _InterlockedExchange(&sd_state::g_unloading, 1);
                    req->status = STATUS_SUCCESS;
                    SD_LOG("SHUTDOWN quiesce armed by client pid=%llu",
                ...
                status = req->status;
                bytes = sizeof(_SD_SHUTDOWN);
            }
            else {
                status = STATUS_INFO_LENGTH_MISMATCH;
            }
        }
        else if (code == ioctl_codes::LOGCTL()) {
            if (input_size >= sizeof(_SD_LOGCTL) && output_size >= sizeof(_SD_LOGCTL)) {
                p_sd_logctl_request req = (p_sd_logctl_request)buffer;
                if (req->magic == 0x5D10C0DE) {
                    if (req->log_level > 4) {
                        req->status = STATUS_INVALID_PARAMETER;
                    }
                    else {
                        // Apply FIRST, then re-read: the old code refreshed
                        // req->log_level from current_log_level() before
                        // applying, so the requested value was always
                        // overwritten with the current one and the level
                        // could never actually change.
                        dbg_capture::set_log_level(req->log_level);
                        if (req->log_cap_mb >= 1 && req->log_cap_mb <= 512) {
                            dbg_capture::set_log_cap_mb(req->log_cap_mb);
                        }
                        req->log_level = dbg_capture::current_log_level();
                        req->log_cap_mb = dbg_capture::current_log_cap_mb();
                        req->status = STATUS_SUCCESS;
                    }
                }
                else if (req->magic != 0) {
            ...
            if (input_size >= sizeof(_RC) && output_size >= sizeof(_RC)) {
                ...
                SD_LOG("DISPATCH_RC_ENTER raw_code=0x%08lx input_size=%lu output_size=%lu requestor=%u requestor_pid=%llu current_pid=%llu current_tid=%llu irql=%lu dtb=0x%llx fn=0x%llx shellcode=0x%llx spoof=0x%llx original_rip=0x%llx fingerprint=0x%llx",
                    code,
                    input_size,
                    output_size,
                ...
                bytes = sizeof(_RC);
                SD_LOG("DISPATCH_RC_EXIT status=0x%08lx bytes=%lu dtb=0x%llx fn=0x%llx entry=0x%llx trampoline=0x%llx completed=%llu result=0x%llx fingerprint_before=0x%llx fingerprint_after=0x%llx",
                ...
                SD_LOG("DISPATCH_RC_SIZE_MISMATCH input_size=%lu output_size=%lu required=%llu requestor_pid=%llu",
                    input_size,
                    output_size,
            ...
            if (input_size >= sizeof(_CR) && output_size >= sizeof(_CR)) {
                ...
                SD_LOG("DISPATCH_CR_ENTER raw_code=0x%08lx input_size=%lu output_size=%lu requestor=%u requestor_pid=%llu current_pid=%llu current_tid=%llu irql=%lu dtb=0x%llx result_addr=0x%llx completed=%llu result=0x%llx fingerprint=0x%llx",
                    code,
                    input_size,
                    output_size,
                    static_cast<ULONG>(irp->RequestorMode),
                ...
                bytes = sizeof(_CR);
                SD_LOG("DISPATCH_CR_EXIT status=0x%08lx bytes=%lu dtb=0x%llx result_addr=0x%llx completed=%llu result=0x%llx fingerprint_before=0x%llx fingerprint_after=0x%llx",
                    static_cast<ULONG>(status),
                    bytes,
                    cr_req->dtb,
                    cr_req->result_address,
                ...
                SD_LOG("DISPATCH_CR_SIZE_MISMATCH input_size=%lu output_size=%lu required=%llu legacy_required=%llu requestor_pid=%llu",
                    input_size,
                    output_size,
            ...
            if (input_size >= sizeof(protect_memory) && output_size >= sizeof(protect_memory)) {
                ...
                SD_LOG("memory::protect_memory: ENTER pid=%lu addr=0x%016llX size=0x%llX new=0x%08X input_size=%lu output_size=%lu",
                    (ULONG)pm_req->pid,
                ...
                SD_LOG("memory::protect_memory: EXIT pid=%lu addr=0x%016llX size=0x%llX new=0x%08X old=0x%08X status=0x%08X bytes=%lu",
                    (ULONG)pm_req->pid,
                    (unsigned long long)pm_req->address,
                    (unsigned long long)pm_req->size,
                    (ULONG)pm_req->new_protect,
                ...
                SD_LOG("memory::protect_memory: SIZE_MISMATCH input_size=%lu output_size=%lu expected=%llu",
            ...
            if (input_size >= sizeof(packet_mod_rule_list) && output_size >= sizeof(packet_mod_rule_list) &&
                ...
                SD_LOG("netaction::PMOD_DISPATCH ENTER kind=list code=0x%08X input_size=%lu output_size=%lu op=%u",
                    code,
                    input_size,
                    output_size,
                    req->operation);
                ...
                SD_LOG("netaction::PMOD_DISPATCH EXIT kind=list status=0x%08X bytes=%lu rule_count=%u",
                    status,
                    bytes,
                    req->rule_count);
            }
                ...
                SD_LOG("netaction::PMOD_DISPATCH ENTER kind=rule code=0x%08X input_size=%lu output_size=%lu op=%u rule_id=%u direction=%u protocol=%u port=%u pid=%u pattern_size=%u replace_size=%u",
                    code,
                    input_size,
                    output_size,
                    req->operation,
                ...
                SD_LOG("netaction::PMOD_DISPATCH EXIT kind=rule status=0x%08X bytes=%lu op=%u rule_id=%u active=%u",
                    status,
                    bytes,
                    req->operation,
                    req->rule_id,
            ...
            SD_LOG("netaction::STRM ENTER code=0x%08X input_size=%lu output_size=%lu required=%lu buffer=%p",
            ...
            if (input_size >= sizeof(stream_reassemble_request) && output_size >= sizeof(stream_reassemble_request)) {
                ...
                SD_LOG("netaction::STRM dispatch op=%u src_port=%u dst_port=%u pid=%u",
                ...
                SD_LOG("netaction::STRM EXIT status=0x%08X bytes=%lu stream_size=%u total_packets=%u stream_count=%u truncated=%u",
                    status, bytes, strm_req->stream_size, strm_req->total_packets,
                    strm_req->stream_count, strm_req->truncated);
            }
            else {
                status = STATUS_INFO_LENGTH_MISMATCH;
                SD_LOG("netaction::STRM REJECT length_mismatch input_size=%lu output_size=%lu required=%lu status=0x%08X",
            ...
            SD_LOG("netaction::CKIL ENTER code=0x%08X input_size=%lu output_size=%lu required=%lu buffer=%p",
            ...
            if (input_size >= sizeof(conn_kill_request) && output_size >= sizeof(conn_kill_request)) {
                ...
                SD_LOG("netaction::CKIL dispatch protocol=%u af=%u src_port=%u dst_port=%u pid=%u src=%u.%u.%u.%u dst=%u.%u.%u.%u",
                ...
                SD_LOG("netaction::CKIL EXIT status=0x%08X bytes=%lu request_status=%u",
                    status, bytes, ckil_req->status);
            }
            else {
                status = STATUS_INFO_LENGTH_MISMATCH;
                SD_LOG("netaction::CKIL REJECT length_mismatch input_size=%lu output_size=%lu required=%lu status=0x%08X",
            ...
            SD_MALSAFE_LOG_INFO("ioctl PSBX ENTRY caller=%lu input=%lu output=%lu need=%lu",
            ...
            if (input_size >= sizeof(protect_sandbox_request_k) &&
                ...
                status = STATUS_SUCCESS;
                SD_MALSAFE_LOG_INFO("ioctl PSBX EXIT pid=%lu flags=0x%08X result=%d denials=%llu status=0x%08X",
                    req->pid, req->flags, ok ? 1 : 0, (unsigned long long)req->denials_so_far, (ULONG)status);
                bytes = sizeof(protect_sandbox_request_k);
            } else {
                SD_MALSAFE_LOG_WARN("ioctl PSBX REJECT length_mismatch input=%lu output=%lu need=%lu",
            ...
            SD_MALSAFE_LOG_INFO("ioctl USBX ENTRY caller=%lu input=%lu output=%lu need=%lu",
            ...
            if (input_size >= sizeof(protect_sandbox_request_k) &&
                ...
                req->denials_so_far = static_cast<UINT64>(denials);
                status = STATUS_SUCCESS;
                SD_MALSAFE_LOG_INFO("ioctl USBX EXIT pid=%lu result=%d denials=%llu status=0x%08X",
                    req->pid, ok ? 1 : 0, (unsigned long long)req->denials_so_far, (ULONG)status);
                bytes = sizeof(protect_sandbox_request_k);
            } else {
                SD_MALSAFE_LOG_WARN("ioctl USBX REJECT length_mismatch input=%lu output=%lu need=%lu",
                    input_size, output_size, (ULONG)sizeof(protect_sandbox_request_k));
                status = STATUS_INFO_LENGTH_MISMATCH;
            }
        }
            ...
            SD_MALSAFE_LOG_INFO("ioctl NLOG ENTRY caller=%lu input=%lu output=%lu need=%lu",
            ...
            if (input_size >= sizeof(net_log_register_request_k) &&
                ...
                req->result = ok ? 1u : 0u;
                status = STATUS_SUCCESS;
                SD_MALSAFE_LOG_INFO("ioctl NLOG EXIT pid=%lu op=%lu result=%d status=0x%08X",
                    req->pid, req->operation, ok ? 1 : 0, (ULONG)status);
                bytes = sizeof(net_log_register_request_k);
            } else {
                SD_MALSAFE_LOG_WARN("ioctl NLOG REJECT length_mismatch input=%lu output=%lu need=%lu",
            ...
            SD_MALSAFE_LOG_VERBOSE("ioctl NPKT entry input=%lu output=%lu rec_size=%lu",
            ...
            if (input_size < sizeof(net_packet_pull_request_k) ||
                ...
                status = STATUS_SUCCESS;
                SD_MALSAFE_LOG_INFO("ioctl NPKT pid=%lu max=%lu returned=%lu dropped_since=%llu out_bytes=%lu",
